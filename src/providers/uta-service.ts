...
export class UtaService {
  ...
  login(rut: string, clave: string) {
    ...
    .then( data => {
      ...
      if (data.estado == '1' && data.error == ""){
        this.sendUserId(data.idUsuario); //ENVIO EL USER_ID DE ONESIGNAL AL SERVIDOR
  ...
  logout(): Promise<any>{ //ARREGLAR ESTO
    ...
    return this._storage.getOneSignalId()
      .then( onesignal_id => {
        ...
        .then( university_id => {
          ...
          this.http.get(URLs.base + "cerrar-sesion", { search: params }) //SI RESPONDE CORRECTAMENTE BORRO EL STORAGE
          .map( res => res.json()) //MAPEAR RESPUESTA PARA QUE SEA UN OBJETO CON SOLO LA DATA
